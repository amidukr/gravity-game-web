...
{
	// default ray dir
	vec3 dir = _position - cameraPosition;
	...
	// sun light dir
	vec3 l = _position - starPosition;
			  
	vec2 e = ray_vs_sphere( eye, dir, planetRadius );
	if ( e.x > e.y ) {
		gl_FragColor = vec4( 0.0, 0.0, 0.0, 1.0 );
        return;
	}
	
	vec2 f = ray_vs_sphere( eye, dir, planetRadius + atmosphereHeight );
	e.y = min( e.y, f.x );
