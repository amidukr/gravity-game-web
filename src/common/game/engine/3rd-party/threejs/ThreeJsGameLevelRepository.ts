...
export class ThreeJsGameLevelRepository implements GameLevelRepository {
  ...
  async loadLevel(levelDescriptor: SimpleGameLevelDescriptor): Promise<GravityGameLevelObject> {
    ...
    const gameScenePromise = new Promise<GLTF>((r) => gtlfLoader.load(`${levelObject.resourceRootFolder}/${levelObject.data.levelSceneFile}`, r));
    ...
    if (levelObject.data.backgroundFiles.length == 6) {
      ...
      levelObject.backhroundTexture = new TextureLoader().load(`${levelObject.resourceRootFolder}/${levelObject.data.backgroundFiles[0]}`);
