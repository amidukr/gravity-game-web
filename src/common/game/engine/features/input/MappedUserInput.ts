...
export class MappedUserInput implements ApplicationComponent {
  ...
  private getOrCreateGroupActionToHotKeysMapping(group: InputMappingGroup, action: InputAction): GroupActionToHotKeysMappings {
    ...
    return actionToHotkeyMapping.actions[action.actionId] || (actionToHotkeyMapping.actions[action.actionId] = { hotKeys: [] });
  }
  ...
  isActionPressed(gameView: GameView, group: InputMappingGroup, action: InputAction): boolean {
    ...
    return (
      this.actionToHotKeysMapping.groups[group.groupId]?.actions?.[action.actionId]?.hotKeys.find((x) => buttonUserInput.isButtonPressed(x.button)) != undefined
    );
  }
